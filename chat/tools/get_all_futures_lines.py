...
def process_odds_table(odds_table, league: str | None, book_name: str, bet_type: str, team: str | None, player: str | None, log_extras, one_sided: bool = False):
    ...
    if len(odds_table) == 0:
        ...
        string = "get_all_futures_lines returned 0 results.  YOU MUST TELL THE USER THAT YOU CURRENTLY DO NOT HAVE ACCESS TO LIVE FUTURES LINES OR DIRECT LINKS TO THE BOOKS FOR FUTURES RIGHT NOW."
        ...
        if one_sided:
            ...
            try:
                # drop: home_over_yes_market_selection_id, home_over_yes_book_id, visitor_under_no_market_selection_id, visitor_under_no_book_id
                odds_table = odds_table.drop(columns=['market_offer_id', 'home_over_yes_market_selection_id', 'visitor_under_no_market_selection_id', 'book_id', 'main'], errors='ignore')
            except:
                pass
        # SWITCH UTC TO EST
...
async def get_all_futures_lines(league=None, bet_type="all", book_name="all", team=None, player=None, log_extras={}, **kwargs):
    ...
    try:
        if one_sided:
            ...
            odds_table = await get_table("odds_table_futures_two_sided_all_books")
        ...
        odds_table = await asyncio.to_thread(process_odds_table, odds_table, league, book_name, bet_type, team, player, log_extras, one_sided)
    ...
    if len(odds_table) > 0: odds_table['tool_used'] = "get_all_futures_lines"
...
# try:
#     odds_table, models_used_array = asyncio.run(get_all_futures_lines(
#         league="NFL",
#         bet_type="NFL Regular Season 2024/25 - Future Team Prop Total",
