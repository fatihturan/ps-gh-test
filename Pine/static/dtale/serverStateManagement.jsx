...
function renameColumn(dataId, col, rename, callback) {
  fetchJson(buildURLString(`/dtale/rename-col/${dataId}/${encodeURIComponent(col)}`, { rename }), callback);
}
...
function editCell(dataId, col, rowIndex, updated, callback) {
  fetchJson(
    buildURLString(`/dtale/edit-cell/${dataId}/${encodeURIComponent(col)}`, {
      rowIndex,
      updated,
    }),
    callback
...
export default {
  ...
  updateSettings,
  deleteColumn: (dataId, col) => () => fetchJson(buildURLString(`/dtale/delete-col/${dataId}/${encodeURIComponent(col)}`), _.noop),
  renameColumn,
  updateFormats,
  editCell,
  updateDisplay,
