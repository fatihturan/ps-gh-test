    ...
    **kwargs
    ...
    if group_col is not None and len(group_col):
        ...
        if len(group_vals) > MAX_GROUPS:
            ...
            msg = (
                "The group you chose contains a lot of values.  Putting all of these in a visualization will make it hard to read. "
                'You can choose specific groups to display from then "Group(s)" dropdown above. The available options are also listed below.'
            )
            #msg = (
            #   "The group you chose ({}) contains more than {} unique values, more groups than that will make the chart hard to read. "
