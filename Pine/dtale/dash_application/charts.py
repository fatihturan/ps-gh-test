...
def build_spaced_ticks(ticktext, mode="auto"):
    ...
    #originall - (size / 28.0) and tick_cutoff = 30
    ...
    if mode == "array":
        ...
        spaced_ticks, spaced_text = [tickvals[size - 1]], [ticktext[size - 1]]

        #originally for i in range(factor, size - 1, factor), reversed so that there should be no overlap in the ticks of the top players
        for i in range((size - 1) - factor, 0, -factor):
            #added if statement so there is no overlap in ticks on the lower end
            if i > (2/3 * factor):
                spaced_ticks.append(tickvals[i])
                spaced_text.append(ticktext[i])

        #No need for this anymore since we are iterating through the list backwards
        #spaced_ticks.append(tickvals[-1])
        #spaced_text.append(ticktext[-1])
        #adding 0 value since we are iterating through the list backwards
        spaced_ticks.append(tickvals[0])
        spaced_text.append(ticktext[0])
...
def chart_wrapper(data_id, data, url_params=None):
    ...
    def _chart_wrapper(chart, group_filter=None):
        ...
        def build_url(path, query):
            #debug
            logger_chart_url = fix_url_path("/charts/{}?{}".format(data_id, query))

            #Everytime a chart is made, this runs twice to build the export png and save links.
            #Just need it once for logging purposes
            if path == '/dtale/chart-export':
                logger.info("{} {} {} {}".format(request.method, logger_chart_url, get_ip(request), data_id.split("_")[0]))
        ...
        export_html_link = build_hoverable(
            html.A(
                html.I(className="fas fa-save mr-4 chart-button"),
                href=build_url("/file_send_server", querystring),
        ...
        export_png_link = build_hoverable(
            html.A(
                html.I(className="fas fa-camera mr-4 chart-button"),
...
def build_layout(cfg):
    ...
    return go.Layout(
        **dict_merge(
            dict(legend=dict(orientation="h"),
                 #plot_bgcolor="#FFFFFF",
                 #xaxis = dict(gridcolor="F4F4F4"),
                 #yaxis = dict(gridcolor="F4F4F4"),
                 #font = dict(color="#112f55"),
                 # This uses px.colors.qualitative.D3 colorscale but adds our green to the beginning
                 #D3 colorscale is '#1F77B4', '#FF7F0E', '#2CA02C', '#D62728', '#9467BD', '#8C564B', '#E377C2', '#7F7F7F', '#BCBD22', '#17BECF']
                 #first ten are colorway in regular order, second ten are 2,1,4,3,6,5,8,7,10,9 this gets you ten different colors if you are doing a trendline since trendline uses every other colorway
                 colorway=['#4EC48F', '#4079b9', '#ffa500', '#a500ff', '#0099b3', '#ff5e13', '#8040b9', '#aecc00', '#959595', '#b94079', '#4079b9', '#4EC48F', '#a500ff', '#ffa500', '#ff5e13', '#0099b3', '#aecc00', '#8040b9', '#b94079', '#959595']
                 ),
            cfg))
...
def build_scatter_trendline(x, y, trendline, data_id, x_col, y_col, trendline_color):
    ...
    fig = px.scatter(
        #Original trendline_color_override was "#c0d3e9"
        x=_format_data(x, x_col), y=_format_data(y, y_col), trendline=trendline, trendline_color_override=trendline_color
    )
    return fig.data[1]

#changed size from 8 to 6 to match export
def build_scatter_marker(series, z, group, colorscale=None):
    if z is not None:
        return {
            "size": 6,
            "color": series[z],
    ...
    #changed size from 15 to 9 to match export
    return {"size": 9, "line": {"width": 0.5, "color": "white"}}
...
def scatter_builder(
    ...
    def _build_final_scatter(y_val):
        def _build_data():
            #Due to glitch, trendline colors is related to colorway as follows: 1,3,5,7,2,4,6,8
            trendline_colors = ["#d2f0e3", "#ffd6c4", "#c7f7ff", "#e9ddf3", "#e3e3e3", "#cfdeee", "#eac4ff", "#ffd6c4", "#f2ffa5", "#f3dde8"]
            trendline_counter = 0
            ...
            for series_key, d in data["data"].items():
                ...
                # Start of new code
                # Can't get trendline working with date time
                # So this code converts datetime to int with yearmonthday
                #if trendline:
                #    if pd.DataFrame(d["x"])[0].dtype == 'object':
                #        d["x"] = pd.to_datetime(pd.DataFrame(d["x"])[0]).dt.strftime('%Y.%m%d').astype(float).values
                #    if pd.DataFrame(d[y_val])[0].dtype == 'object':
                #        d[y_val] = pd.to_datetime(pd.DataFrame(d[y_val])[0]).dt.strftime('%Y.%m%d').astype(
                #            float).values
                # End of new code
                ...
                yield scatter_func(
                    **dict_merge(
                        dict(
                            ...
                            opacity=0.7,
                            name=series_key,
                            marker=build_scatter_marker(d, z, group, colorscale),
                        ),
                ...
                if trendline:
                    yield build_scatter_trendline(
                        d["x"], d[y_val], trendline, data_id, x, y_val, trendline_colors[trendline_counter]
                    )

                    trendline_counter = trendline_counter + 1

                    if trendline_counter == 10:
                        trendline_counter = 0
                    ...
                    if y_val in series and (group is None or group == series_key):
                        yield scatter_func(
                            **dict(
                                ...
                                opacity=0.7,
                                name=series_key,
                                marker=build_scatter_marker(series, z, group, colorscale),
...
def export_chart(data_id, params):
    ...
    #This should get me the name of the tuple in data (e.g., scattergl)
    #print(type(chart['data'][0]).__name__, flush=True)
    #Reduces scatter marker from 15 to 8.  Note scatter3d marker is already 8.
    #This does not distinguish between scatter and scatter3d

    #Optimizes for mobile - reduces margins, reduces font size, and removes title
    #top margin was 25 to make room for plotly menu. Removed plotly menu in css in app.py
    chart['layout']['margin'] = dict(l=2, r=2, t=2, b=2)
    chart['layout']['font'] = dict(size=14,family='Raleway')
    chart['layout'].pop('title')
    chart['layout']['dragmode'] = False
    #Old code - checks for marker -
    #if 'marker' in chart['data'][0]:

    #Gets how many columns are being used in the chart
    number_of_columns = np.asarray(chart["data"]).shape[0]

    #tries to remove y axis label if there is a comma (that means there is a legend and no need for y axis)
    try:
        if ',' in chart['layout']['yaxis']['title']['text']:
            print("Multipy y axes.  Using legend instead of y label.", flush=True)
            chart['layout']['yaxis']['title']['text'] = ''
            chart['layout']['margin'] = dict(l=0, r=2, t=2, b=2)
    except:
        print("Exception looking for y axis text", flush=True)
        pass

    #Handles legend info - tries a few things, passes if it doesn't work
    try:
        try:
            #tries to check if the only thing in the legend is "all."  if so, removes legend
            if chart["data"][0]["name"] == 'all':
                chart['layout']['showlegend'] = False
            #else tries to count length of largest string in x-axis and align legend accordingly.
            else:
                try:
                    for x in chart['layout']['xaxis']['ticktext']:
                        max_len = 0
                        if len(str(x)) > max_len:
                            max_len = len(str(x))
                        # legend is determined by max length of x axis
                    chart['layout']['legend']['y'] = -(max_len / 30) + (-.07 * (math.floor((number_of_columns / 2) - 1)))
                    #makes sure y is <=-.3
                    if chart['layout']['legend']['y'] >= -.3: chart['layout']['legend']['y'] = -.3
                except:
                    #if not, go with x
                    ...
                    #makes sure y is <=-.3
                    if chart['layout']['legend']['y'] >= -.3: chart['layout']['legend']['y'] = -.3
        # if there's an error trying to find "all", still tries to count length of largest string in x-axis and align legend accordingly.
        except:
            try:
                #check if there's tick text
                for x in chart['layout']['xaxis']['ticktext']:
                    max_len = 0
                    if len(str(x)) > max_len:
                        max_len = len(str(x))
                ...
                #makes sure y is <=-.3
                if chart['layout']['legend']['y'] >= -.3: chart['layout']['legend']['y'] = -.3
    #if there's any error doing any of the above, just moves on.
    except:
        pass

    # Optimizes Line (scatter) for mobile -  changes colors
    if type(chart['data'][0]).__name__ == 'Scatter':

        #optimizes colors
        chart['layout']['plot_bgcolor'] = '#ffffff'
        chart['layout']['xaxis']['linecolor'] = '#274970'
        chart['layout']['xaxis']['gridcolor'] = '#e9edf1'
        chart['layout']['yaxis']['gridcolor'] = '#e9edf1'

    #Optimizes Scatter Plot (scattergl) for mobile - reduces markersize to 11, changes colors
    if type(chart['data'][0]).__name__ == 'Scattergl':

        #optimizes for mobile
        chart['data'][0]['marker']['size'] = 9
    ...
    #Optimizes Bar (dict) for mobile - changes colors
    if type(chart['data'][0]).__name__ == 'dict':
        ...
        #chart['layout']['showlegend'] = False
        #chart['layout']['legend']['font']['size'] = 5
        #chart['layout']['legend']['orientation'] = "v"
        #chart['layout']['legend']['x'] = 0

    #Optimizes 3d Scatter (Scatter3d) for mobile - reduces markersize to 5, changes colors
    if type(chart['data'][0]).__name__ == 'Scatter3d':

        #optimizes for mobile
        chart['data'][0]['marker']['size'] = 6

        #changes font size to 10 - cannot be dynamically changed in css
        chart['layout']['font'] = dict(size=10, family='Raleway')

        #increases top margin to make room for menu
        chart['layout']['margin'] = dict(l=2, r=2, t=25, b=15)

        #optimizes colors
        chart['layout']['plot_bgcolor'] = '#ffffff'
        chart['layout']['scene']['bgcolor'] = '#ffffff'
        chart['layout']['scene']['xaxis']['backgroundcolor'] = '#ffffff'
        chart['layout']['scene']['xaxis']['gridcolor'] = '#e9edf1'
        chart['layout']['scene']['yaxis']['backgroundcolor'] = '#ffffff'
        chart['layout']['scene']['yaxis']['gridcolor'] = '#e9edf1'
        chart['layout']['scene']['zaxis']['backgroundcolor'] = '#ffffff'
        chart['layout']['scene']['zaxis']['gridcolor'] = '#e9edf1'
    ...
    write_html(
        chart,
        file=html_buffer,
        #gets js from external site, should save space
    ...
    #removes (No Aggregation)
    html_str = html_str.replace(' (No Aggregation)', '')

    #Adds google font and css
    #Keeps top menu for 3d charts
    if type(chart['data'][0]).__name__ == 'Scatter3d':
        html_str = html_str.replace('</head>', '<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no"><link rel="preconnect" href="https://fonts.gstatic.com"><link href="https://fonts.googleapis.com/css2?family=Raleway:wght@400;600;700&display=swap" rel="stylesheet"></head><style>@media screen and (min-width: 656px) {text{font-size: 14px!important;} .legendtext{font-size: 9px!important;} } @media screen and (max-width: 655px) {text{font-size: 9px!important;} .legendtext{font-size: 7px!important;}}</style>')

    #Removes top menu for other charts
    else:
        html_str = html_str.replace('</head>', '<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no"><link rel="preconnect" href="https://fonts.gstatic.com"><link href="https://fonts.googleapis.com/css2?family=Raleway:wght@400;600;700&display=swap" rel="stylesheet"></head><style>.modebar { display: none !important; } @media screen and (min-width: 656px) {text{font-size: 14px!important;} .legendtext{font-size: 9px!important;} } @media screen and (max-width: 655px) {text{font-size: 9px!important;} .legendtext{font-size: 7px!important;}}</style>')
...
def export_png(data_id, params):
    chart = build_raw_chart(data_id, export=True, **params)
    #Use pine logo from our website
    #pinesportslogo imgur link - https://i.imgur.com/z4mRIXQ.png
    #inverted logo imgur link - https://i.imgur.com/nXSLDkq.png
    #old h/w .15
    chart['layout']['images'] = [dict(x=.08, y=1.02, sizex=.15, sizey=.15, source="https://i.imgur.com/z4mRIXQ.png", xanchor="right", xref="paper", yanchor="bottom", yref="paper", layer="below")]
    chart['layout']['font'] = dict(size=14)
    chart['layout'].pop('title')
    #Gets how many columns are being used in the chart
    number_of_columns = np.asarray(chart["data"]).shape[0]
    ...
    write_image(chart, file=image_buffer, width=1050, height=750, format="png")
    image_buffer.seek(0)
