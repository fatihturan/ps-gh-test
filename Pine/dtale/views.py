...
def view_main(data_id=None):
    ...
    logger.info("{} {} {} {}".format(request.method, request.environ.get('REQUEST_URI'), get_ip(request), data_id.split("_")[0]))
...
def describe(data_id, column):
    ...
    """
    columnFixed = unquote_plus(column)
    data = global_state.get_data(data_id)[[columnFixed]]
    additional_aggs = None
    dtype = global_state.get_dtype_info(data_id, columnFixed)
    if classify_type(dtype["dtype"]) in ["I", "F"]:
        additional_aggs = ["sum", "median", "mode", "var", "sem", "skew", "kurt"]
    code = build_code_export(data_id)
    desc, desc_code = load_describe(data[columnFixed], additional_aggs=additional_aggs)
    code += desc_code
    ...
    uniq_vals = data[columnFixed].value_counts().sort_values(ascending=False)
...
def variance(data_id, column):
    ...
    """
    columnFixed = unquote_plus(column)
    s = global_state.get_data(data_id)[columnFixed]
    code = ["s = df['{}']".format(columnFixed)]
    unique_ct = unique_count(s)
...
def outliers(data_id, column):
    ...
    formatter = find_dtype_formatter(find_dtype(df[columnFixed]))
    ...
    query = build_outlier_query(iqr_lower, iqr_upper, s.min(), s.max(), columnFixed)
    code = (
        "s = df['{column}']\n"
        "q1 = s.quantile(0.25)\n"
        "q3 = s.quantile(0.75)\n"
    ...
    ).format(column=columnFixed)
    queryApplied = columnFixed in (
...
def toggle_outlier_filter(data_id, column):
    ...
    if columnFixed in outlierFilters:
        settings["outlierFilters"] = {
            k: v for k, v in outlierFilters.items() if k != columnFixed
        }
    else:
        dtype_info = global_state.get_dtype_info(data_id, columnFixed)
        ...
        query = build_outlier_query(iqr_lower, iqr_upper, min_val, max_val, columnFixed)
        settings["outlierFilters"] = dict_merge(
            outlierFilters, {columnFixed: {"query": query}}
        )
...
def delete_col(data_id, column):
    columnFixed = unquote_plus(column)
    data = global_state.get_data(data_id)
    data = data[[c for c in data.columns if c != columnFixed]]
    curr_history = global_state.get_history(data_id) or []
    curr_history += ["df = df[[c for c in df.columns if c != '{}']]".format(columnFixed)]
    global_state.set_history(data_id, curr_history)
    dtypes = global_state.get_dtypes(data_id)
    dtypes = [dt for dt in dtypes if dt["name"] != columnFixed]
    curr_settings = global_state.get_settings(data_id)
    curr_settings["locked"] = [
        c for c in curr_settings.get("locked", []) if c != columnFixed
    ]
    global_state.set_data(data_id, data)
...
def rename_col(data_id, column):
    ...
    data = global_state.get_data(data_id)
    if columnFixed != rename and rename in data.columns:
        return jsonify(error='Column name "{}" already exists!')

    data = data.rename(columns={columnFixed: rename})
    curr_history = global_state.get_history(data_id) or []
    curr_history += ["df = df.rename(columns={'%s': '%s'})" % (columnFixed, rename)]
    global_state.set_history(data_id, curr_history)
    dtypes = global_state.get_dtypes(data_id)
    dtypes = [
        dict_merge(dt, {"name": rename}) if dt["name"] == columnFixed else dt
        for dt in dtypes
    ]
    curr_settings = global_state.get_settings(data_id)
    curr_settings["locked"] = [
        rename if c == columnFixed else c for c in curr_settings.get("locked", [])
    ]
    global_state.set_data(data_id, data)
    global_state.set_dtypes(data_id, dtypes)
    global_state.set_settings(data_id, curr_settings)
...
def edit_cell(data_id, column):
    ...
    dtype = find_dtype(data[columnFixed])

    code = []
    if updated in ["nan", "inf"]:
        updated_str = "np.{}".format(updated)
        updated = getattr(np, updated)
        data.loc[row_index, columnFixed] = updated
        code.append(
            "df.loc[{row_index}, '{column}'] = {updated}".format(
                row_index=row_index, column=columnFixed, updated=updated_str
            )
        )
    else:
        classification = classify_type(dtype)
        if classification == "B":
            ...
            if dtype.startswith("category") and updated not in data[columnFixed].unique():
                data[columnFixed].cat.add_categories(updated, inplace=True)
                code.append(
                    "data['{column}'].cat.add_categories('{updated}', inplace=True)".format(
                        column=columnFixed, updated=updated
                    )
                )
            updated_str = "'{}'".format(updated)
        data.at[row_index, columnFixed] = updated
        code.append(
    ...
    ranges = {}
    try:
        ranges[columnFixed] = data[[columnFixed]].agg(["min", "max"]).to_dict()[columnFixed]
    except ValueError:
        pass
    dtype_f = dtype_formatter(data, {columnFixed: dtype}, ranges)
    dtypes = [
        dtype_f(dt["index"], columnFixed) if dt["name"] == columnFixed else dt for dt in dtypes
    ]
    global_state.set_dtypes(data_id, dtypes)
...
def get_column_filter_data(data_id, column):
    ...
    if classification in ["S", "I", "B"]:
        ret["uniques"] = build_filter_vals(s, data_id, columnFixed, fmt)
...
def save_column_filter(data_id, column):
    ...
    curr_filters = ColumnFilter(
        data_id, columnFixed, get_str_arg(request, "cfg")
    ).save_filter()
...
def get_scatter(data_id):
    ...
    if len(data) > 15000:
        return jsonify(
            stats=stats,
            code="\n".join(code),
            error="Dataset exceeds 15,000 records.  If you apply a filter, you will be able to see additional visualizations.",
        )
...
def build_chart_filename(chart_type, ext="html", title=""):

    #creates new date_time format that is easy to parse
    return "{}_-_{}_-_{}_Chart.{}".format(
        pd.Timestamp("now").strftime('%Y-%m-%d_-_%H~%M~%S'), title, chart_type, ext
    )

    #old code without title
